...
DATA_DIR = APP_DIR / "us_sensus"   # adjust to your dataset folder

st.set_page_config(page_title="RAG Demo: ObjectBox + Embeddings + LLM Picker", page_icon="🗂️")
st.title("🔎 RAG with ObjectBox, Ollama/HuggingFace Embeddings, and LLM Picker")
...
def build_index():
    ...
    if embed_choice.startswith("ollama:"):
        ...
        emb_dim = 768  # most Ollama embedding models are 768-d

    elif embed_choice.startswith("huggingface:"):
        from langchain_community.embeddings import HuggingFaceEmbeddings
        emb_model = embed_choice.split("huggingface:", 1)[1]
        st.session_state.embeddings = HuggingFaceEmbeddings(model_name=emb_model)

        # set dimensions based on known models
        if "MiniLM" in emb_model:
            emb_dim = 384
        elif "base" in emb_model:
            emb_dim = 768
        elif "large" in emb_model:
            emb_dim = 1024
        else:
            emb_dim = 768  # safe default
...
if input_prompt:
    ...
    # A bit more forgiving retriever
